...
public class ImageEditService {
    ...
    /**
     * 使用通义千问 Image Generation 模型生成新图片
     *
     * @param prompt 图片生成提示词，描述你想要的图像效果
     * @return 生成的图片的 URL
     * @throws RuntimeException 如果请求失败或返回错误
     */
    public String createImage(String prompt) {
        ...
        // 构建 content 数组
        JSONArray contentArray = new JSONArray();
        contentArray.add(JSONUtil.createObj().set("text", prompt));
        ...
        log.info("正在发送图片生成请求，请求体: {}", requestBody.toStringPretty());

        try (HttpResponse response = HttpRequest.post(apiUrl)
                .header("Content-Type", "application/json")
                .header("Authorization", "Bearer " + apiKey)
                .body(requestBody.toString())
                .timeout(60000) // 60秒超时，图片生成可能较慢
                .execute()) {
            ...
            if (response.isOk()) {
                ...
                if (jsonResponse.containsKey("output")) {
                    ...
                    if (choices != null && choices.size() > 0) {
                        ...
                        if (StrUtil.isNotBlank(imageUrl)) {
                            log.info("✅ 图片生成成功，结果图片 URL: {}", imageUrl);
                            return imageUrl;
                        } else {
                            throw new RuntimeException("API 返回成功，但未找到图片 URL");
                        }
            ...
            log.error("调用通义千问图片生成服务时发生异常", e);
            throw new RuntimeException("图片生成请求失败: " + e.getMessage(), e);
        }
    }
}